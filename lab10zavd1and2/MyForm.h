...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	// Display the result
	richTextBox1->AppendText("Кількість додатних елементів: " + Convert::ToString(positiveCount) + "\n");
}
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		{
			MessageBox::Show("Невірний формат вхідних даних або k повинно бути більше нуля.", "Помилка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			return;
		}

		// Перевірка, що b > a
		if (b <= a) {
			MessageBox::Show("Значення 'b' повинно бути більше за 'a'.", "Помилка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			return;
		}
		...
		for (int i = 0; i <= k; i++) {
			...
			// Перевірка, чи x відповідає області визначення функції
			if (x <= 0) {
				listBox1->Items->Add("\t" + x.ToString("0.00") + "\t|\t Значення не визначене");
				continue;
			}

			try {
				y = sin(log(pow(x, 3)) + abs(x));  // Обчислення функції
				listBox1->Items->Add("\t" + x.ToString("0.00") + "\t|\t" + y.ToString("0.00"));
			}
			catch (Exception^ ex) {
				listBox1->Items->Add("\t" + x.ToString("0.00") + "\t|\t" + "Помилка: " + ex->Message);
			}
		}
	}
	};
